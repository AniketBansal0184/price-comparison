...
const ProductComparison = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Fetch all products (or you can filter by category later)
        const allProducts = await getProducts("", "", 0, 50); // adjust limit as needed
        const foundProduct = allProducts.find((p) => p.id.toString() === id);
        setProduct(foundProduct);

        // Related products: same category if available, else random
  ...
  const bestStore =
    product.stores.find((store) => store.isBestPrice) || product.stores[0];
                      ...
                      >
                        ...
                        <td className="text-center py-4">
                          <div className="flex items-center justify-center space-x-2">
                            <Button
                              variant={
                                store.isBestPrice ? "default" : "outline"
                              }
              ...
              {product.stores
                ...
                .map((store) => (
                  <Card
                    key={store.id}
                    className={
                      store.isBestPrice ? "border-green-200 bg-green-50" : ""
                    }
                  >
